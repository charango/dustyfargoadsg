     ...
     PlanetarySystem *sys;
{
  ...
  for (k=0; k < sys->nb; k++) {
    if (sys->acc[k] > 1e-10) {
      ...
      j_min =(int)((real)ns/(PMAX-PMIN)*(angle - 2.0*RRoche/Rplanet - PMIN));
      j_max =(int)((real)ns/(PMAX-PMIN)*(angle + 2.0*RRoche/Rplanet - PMIN));
...
#pragma omp atomic
      ...
      PyPlanet += dPyPlanet;
      Mplanet  += dMplanet;
      //printf("dMplanet = %lg, dPxPlanet = %lg, dPyPlanet = %lg\n",dMplanet,dPxPlanet,dPyPlanet);      
