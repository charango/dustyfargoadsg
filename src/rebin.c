...
Resample the hydrodynamical fields at a restart with a different
resolution.  Note that at the restart, even if NRAD and NSEC coincide
with previous values, the data is resampled if the radii do not
coincide (for instance, if we switch from ARITHMETIC to LOGARITHMIC
spacing).
...
int nb;
{
  ...
  for (type = 0; type < 5; type++) {
    ...
    case 1: sprintf (radix, "Temperature"); break;
    case 2: sprintf (radix, "vrad"); 
      Old_r = OldRadii; 
      memcpy (New_r, Radii, (GLOBALNRAD+1)*sizeof(double));
      break;
    case 3: sprintf (radix, "vtheta"); dangle = 0.5; break;
