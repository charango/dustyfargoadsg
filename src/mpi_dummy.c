...
Fake MPI functions library for sequential built.  It is used instead
of the true MPI library in the case of a sequential built (see
makefile).
...
void MPI_Comm_rank (a, b)
     ...
     int *b;
{
  *b = 0;			/* Only one process, with rank
				   zero... */
}
...
void MPI_Comm_size (a, b)
     ...
     int *b;
{
  *b = 1;			/* Only one process in the world
				   communicator... */
}
